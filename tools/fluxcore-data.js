...
async function backupDatabaseSet(root, destination) {
  ...
  if (files.length === 0) {
    throw new Error('no Fluxcore SQLite databases were found');
  }
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'verify') {
    if (!argv[1]) {
      throw new Error('usage: node tools/fluxcore-data.js verify <backup-directory>');
    }
  ...
  if (command === 'inspect') {
    ...
    if (results.length === 0) {
      console.log('No live Fluxcore SQLite databases found.');
    }
    for (const result of results) {
      console.log(
  ...
  throw new Error(
    'usage: node tools/fluxcore-data.js <backup [directory] | verify <directory> | inspect>',
  );
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[fluxcore-data] ${error.message}`);
    process.exitCode = 1;
  });
}
