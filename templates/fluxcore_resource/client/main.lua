...
---@class FluxcoreObservedState
---@field job FluxcoreJob?
---@field status string?

---@type FluxcoreObservedState
local observed = {
    job = nil,
    status = nil
}
...
local function debugLog(message)
    if Config.debug then
        print(('[fluxcore_starter] %s'):format(message))
    end
end

---@param job FluxcoreJob?
local function setObservedJob(job)
    ...
    TriggerEvent('fluxcore_starter:client:jobChanged', job, previous)
end
...
local function setObservedStatus(status)
    ...
    TriggerEvent('fluxcore_starter:client:statusChanged', status, previous)
end
...
-- A small local event demonstrating a direct, public Fluxcore Core export call.
-- Trigger it from another client module with:
-- TriggerEvent('fluxcore_starter:client:printPlayer')
RegisterNetEvent('fluxcore_starter:client:printPlayer', function()
