...
for _, player in ipairs(exports['fluxcore_core']:GetPlayers()) do
...
-- add_ace group.admin command.fluxcorestatus allow
RegisterCommand(Config.adminCommand, function(source, args)
    ...
    ---@type FluxcorePlayerSnapshot?
    local player = exports['fluxcore_core']:GetPlayer(target)
    if not player then
        reply(source, ('Player %s has no active Fluxcore character.'):format(
            target
        ))
        return
    end
    ...
    -- Job changes must instead use fluxcore_core:SetJob(), which also persists them.
