...
class FrameworkDatabase {
  ...
  moveMoney(
    ...
    return this.transaction(() => {
      const fromWallet = this.statements.getWallet.get(
        character.internalId,
        fromCurrency,
      );
      const toWallet = this.statements.getWallet.get(
        character.internalId,
        toCurrency,
      );
      ...
      if (fromBalance < 0) {
        throw frameworkError(
          'INSUFFICIENT_FUNDS',
          `insufficient ${fromCurrency}`,
        );
      }
      if (!Number.isSafeInteger(toBalance) || toBalance > 1_000_000_000) {
        throw frameworkError(
          'BALANCE_LIMIT_EXCEEDED',
          `${toCurrency} balance would exceed the allowed limit`,
        );
      }

      this.statements.updateWallet.run(
        fromBalance,
        character.internalId,
        fromCurrency,
      );
  ...
  transferMoney(
    ...
    if (fromCharacter.internalId === toCharacter.internalId) {
      throw frameworkError(
        'VALIDATION_ERROR',
        'money cannot be transferred to the same character',
      );
    }

    return this.transaction(() => {
      const fromWallet = this.statements.getWallet.get(
        fromCharacter.internalId,
        currency,
      );
      const toWallet = this.statements.getWallet.get(
        toCharacter.internalId,
        currency,
      );
      if (!fromWallet || !toWallet) {
        throw frameworkError(
          'CURRENCY_NOT_FOUND',
          `wallet ${currency} does not exist for both characters`,
        );
      }
      ...
      if (!Number.isSafeInteger(toBalance) || toBalance > 1_000_000_000) {
        throw frameworkError(
          'BALANCE_LIMIT_EXCEEDED',
          `${currency} balance would exceed the allowed limit`,
        );
      }
