...
class CoreService {
  ...
  deleteCharacter(source, characterId, confirmation) {
    ...
    this.runtime.emitServer('Fluxcore:server:characterDeleted', playerSource, id);
    return true;
  }
  ...
  selectCharacter(source, characterId) {
    ...
    this.runtime.emitClient(playerSource, 'Fluxcore:client:playerLoaded', snapshot);
    this.runtime.emitServer('Fluxcore:server:playerLoaded', playerSource, snapshot);
    this.runtime.log(
      'info',
  ...
  logout(source) {
    ...
    this.runtime.emitServer(
      'Fluxcore:server:playerLoggedOut',
      playerSource,
      player.characterId,
    );
    context.player = null;
    this.clearPublicState(playerSource);
    this.runtime.emitClient(playerSource, 'Fluxcore:client:playerLoggedOut');
    return true;
  }
  ...
  getCharacterData(identifier) {
    return publicSnapshot(this.resolveCharacter(identifier));
  }
  ...
  resolveCharacter(identifier) {
    const online = this.getPlayer(identifier);
    if (online) {
      return online;
    }
    if (
      ...
      const character = this.database.loadCharacter(identifier);
      if (character) {
        return character;
      }
    }
  ...
  syncMoney(character, currency, balance) {
    const online = this.getPlayer(character.characterId);
    if (!online) {
      return;
    }
    online.money[currency] = balance;
    this.syncOwner(online);
  }

  getMoney(identifier, currency) {
    const character = this.resolveCharacter(identifier);
    const wallet = validateCurrency(currency);
    return this.database.getMoney(character, wallet);
  }

  getMoneyLedger(identifier, currency, limit = 50) {
    const character = this.resolveCharacter(identifier);
    const wallet =
    ...
    if (
      !Number.isSafeInteger(validLimit) ||
      validLimit < 1 ||
      validLimit > 500
    ) {
      throw frameworkError(
        'VALIDATION_ERROR',
        'ledger limit must be an integer from 1 to 500',
      );
    }
    return this.database
      .getLedger(character.characterId)
      .filter((entry) => !wallet || entry.currency === wallet)
      .slice(-validLimit)
      .reverse()
      .map((entry) => ({
  ...
  moveMoney(
    ...
    const character = this.resolveCharacter(identifier);
    const sourceWallet = validateCurrency(fromCurrency);
    const destinationWallet = validateCurrency(toCurrency);
    const validAmount = validateAmount(amount);
    const result = this.database.moveMoney(
      character,
      sourceWallet,
      destinationWallet,
      validAmount,
    ...
    const online = this.getPlayer(character.characterId);
    if (online) {
      online.money[sourceWallet] = result.fromBalance;
      online.money[destinationWallet] = result.toBalance;
      this.syncOwner(online);
    }
    return result;
  }
  ...
  transferMoney(
    ...
    const fromCharacter = this.resolveCharacter(fromIdentifier);
    const toCharacter = this.resolveCharacter(toIdentifier);
    const wallet = validateCurrency(currency);
    const result = this.database.transferMoney(
      fromCharacter,
      toCharacter,
      wallet,
    ...
    this.syncMoney(fromCharacter, wallet, result.fromBalance);
    this.syncMoney(toCharacter, wallet, result.toBalance);
    return result;
  }
  ...
  syncOwner(player) {
    this.runtime.emitClient(
      player.source,
      'Fluxcore:client:playerUpdated',
      publicSnapshot(player),
    );
  }

  setPublicState(source, character) {
    this.runtime.setPlayerState(source, 'Fluxcore:loaded', true, true);
    this.runtime.setPlayerState(
      source,
    ...
    this.runtime.setPlayerState(source, 'Fluxcore:job', clone(character.job), true);
  }

  clearPublicState(source) {
    this.runtime.setPlayerState(source, 'Fluxcore:loaded', false, true);
    this.runtime.setPlayerState(source, 'Fluxcore:characterId', null, true);
    this.runtime.setPlayerState(source, 'Fluxcore:job', null, true);
  }
}
