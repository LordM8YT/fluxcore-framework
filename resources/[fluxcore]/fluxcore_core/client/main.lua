...
local function loadLocale(locale)
    ...
    if not ok or type(parsed) ~= 'table' then
        print(('[fluxcore_core] Locale locales/%s.json is invalid.'):format(locale))
        return nil
    end
    return parsed
end
...
local requestedLocaleName = normalizeLocale(
    GetConvar('fluxcore_locale', config.locale or fallbackLocaleName),
    fallbackLocaleName
)
...
local function reportSpawnDiagnostics()
    ...
    TriggerServerEvent('fluxcore:server:spawnDiagnostics', {
...
local function spawnAt(position)
    ...
    if GetResourceState('spawnmanager') ~= 'started' then
        print('[fluxcore_core] Spawn failed: spawnmanager is not started.')
        return false
    end
    ...
    exports.spawnmanager:spawnPlayer({
        ...
        CreateThread(function()
            ...
            if spawned then
                print(('[fluxcore_core] Spawned player at %.2f, %.2f, %.2f.'):format(
                    x,
                    y,
                    z
                ))
            else
                print('[fluxcore_core] Spawn incomplete: Cfx did not create an active player ped.')
            end
...
local function loadPlayer(snapshot)
    playerData = snapshot
    if GetResourceState('fluxcore_identity') == 'started' then
        TriggerEvent('fluxcore_identity:client:spawnRequested', copy(snapshot))
    else
        spawnAt(snapshot and snapshot.position)
    end
end

RegisterNetEvent('Fluxcore:client:rpcResponse', function(requestId, response)
    ...
    while true do
        ...
        if playerData then
            ...
            if ped ~= 0 and nativeTrue(DoesEntityExist(ped)) then
                ...
                TriggerServerEvent('Fluxcore:server:updatePosition', {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
    ...
    for requestId, resolver in pairs(pending) do
        resolver({
            ...
            error = {
                ...
                message = locale('core.resourceStopped', nil, 'Fluxcore Core stopped.')
            }
        })
        pending[requestId] = nil
    end
