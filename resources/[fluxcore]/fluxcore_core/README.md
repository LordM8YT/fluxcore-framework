...
`fluxcore_core` is the first resource in the Fluxcore Framework. It owns accounts,
characters, online sessions, money, the active-job snapshot, metadata, and last
known positions. Persistent job assignments are owned by `fluxcore_jobs`.
...
Place the resources under a server's `resources/[fluxcore]` directory and add:
...
The SQLite database is created automatically at `data/Fluxcore.sqlite`. The
...
`fluxcore_locale` is replicated because clients and NUI use the same language as
...
local text = exports.fluxcore_core:Locale(
    'vehicles.created',
    { model = 'sultan', plate = 'Fluxcore', source = 7 }
)

local localeName = exports.fluxcore_core:GetLocale()
local identityTranslations = exports.fluxcore_core:GetLocaleData('identity')
```

Pass an English fallback as the third argument for keys owned by another
resource. See the
[localization guide](https://github.com/LordM8YT/Fluxcore-framework/wiki/Localization)
...
local response = exports.fluxcore_core:CreateCharacter({
    slot = 1,
    firstName = 'Kari',
    lastName = 'Fluxcoremann',
    birthDate = '1995-06-15',
    gender = 'unspecified',
    nationality = 'Norwegian'
})

local response = exports.fluxcore_core:SelectCharacter('vrd_0123456789abcdef')
local response = exports.fluxcore_core:DeleteCharacter('vrd_0123456789abcdef')
...
Server resources can listen for `Fluxcore:server:playerLoaded`,
`Fluxcore:server:playerLoggedOut`, `Fluxcore:server:playerDropped`,
`Fluxcore:server:characterDeleted`, and `Fluxcore:server:jobUpdated`. These are local
...
`fluxcore_example` contains an ACE-protected `/grantcash` command. Give trusted
administrators the `Fluxcore.admin` ACE if you want to use it.
...
4. Restart `fluxcore_core` while connected and verify a fresh character
   selection works.
