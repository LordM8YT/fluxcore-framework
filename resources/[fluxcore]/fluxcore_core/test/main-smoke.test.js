...
test('Cfx wiring boots and completes connection, creation, and selection', () => {
  ...
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-main-'));
  ...
  try {
    ...
    assert.equal(netHandlers.has('Fluxcore:server:rpc'), true);
    ...
    const rpc = netHandlers.get('Fluxcore:server:rpc');
    ...
    assert.equal(
      states.some((state) => state.key === 'Fluxcore:loaded' && state.value === true),
      true,
    );
    ...
    eventHandlers.get('onResourceStop')('fluxcore_core');
  } finally {
