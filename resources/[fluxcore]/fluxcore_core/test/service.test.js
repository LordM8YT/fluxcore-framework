...
function createHarness(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-service-'));
...
test('account, character, login, mutation, and logout form one lifecycle', (t) => {
  ...
  assert.equal(
    events.some((event) => event.eventName === 'Fluxcore:client:playerLoaded'),
    true,
  );
  assert.equal(
    states.some(
      (state) =>
        state.key === 'Fluxcore:loaded' &&
        state.value === true &&
  ...
  assert.equal(
    serverEvents.some(
      (event) => event.eventName === 'Fluxcore:server:playerLoaded',
    ),
    true,
  );
  ...
  assert.equal(
    states.some(
      (state) => state.key === 'Fluxcore:loaded' && state.value === false,
    ),
    true,
  );
});
...
test('character deletion requires ownership and exact confirmation', (t) => {
  ...
  assert.equal(
    serverEvents.some(
      (event) =>
        event.eventName === 'Fluxcore:server:characterDeleted' &&
...
test('service exposes wallet moves and transfers to offline characters', (t) => {
  const { service } = createHarness(t);
  service.attachConnection(20, 'license2:sender', ['license2:sender'], 'Sender');
  const sender = service.createCharacter(20, {
    slot: 1,
    firstName: 'Money',
    lastName: 'Sender',
    birthDate: '1990-01-01',
  ...
  service.selectCharacter(20, sender.characterId);
  ...
  const recipient = service.createCharacter(21, {
    slot: 1,
    firstName: 'Money',
    lastName: 'Recipient',
    birthDate: '1990-01-01',
  ...
  service.selectCharacter(21, recipient.characterId);
  service.logout(21);

  const move = service.moveMoney(
    20,
    'cash',
    'bank',
    100,
  ...
  assert.equal(move.fromBalance, 400);
  assert.equal(move.toBalance, 5100);

  const transfer = service.transferMoney(
    20,
  ...
  assert.equal(transfer.fromBalance, 4500);
  assert.equal(transfer.toBalance, 5600);
  assert.equal(service.getMoney(recipient.characterId, 'bank'), 5600);
  assert.equal(
    service.getMoneyLedger(20, 'bank', 1)[0].reference,
    'transfer:service',
  );
});
