...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_properties] [${level}] ${message}`;
...
const integrations = {
  ...
  inventory: {
    registerStash(id, label, slots, maxWeight) {
      ...
      return globalThis.exports.fluxcore_inventory.RegisterStash(
        id,
        label,
        slots,
        maxWeight,
    ...
    openInventory(source, id) {
      requireResource('fluxcore_inventory');
      return globalThis.exports.fluxcore_inventory.OpenInventory(source, id);
    },
  },
};
...
onNet('fluxcore_properties:server:request', (requestId, method, payload) => {
  ...
  runtime.emitClient(
    source,
    'fluxcore_properties:client:response',
