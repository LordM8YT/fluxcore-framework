...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_vehicles] [${level}] ${message}`;
    if (level === 'error') {
...
const genericVehicleAccess = new Map();
const genericLockStates = new Map();
...
function notify(source, text, kind = 'info', code = null) {
  if (Number(source) > 0) {
    runtime.emitClient(
      Number(source),
      'fluxcore_vehicles:client:message',
      String(text),
      kind,
      code,
    );
...
function registerTrunk(vehicle) {
  const response = globalThis.exports.fluxcore_inventory.RegisterContainer(
    `vehicle:${vehicle.id}`,
    'vehicle',
    vehicle.id,
    `${vehicle.plate} trunk`,
...
function spawnFor(source, vehicleId, garageId) {
  ...
  try {
    ...
    runtime.emitClient(
      source,
      'fluxcore_vehicles:client:spawned',
      networkId,
      vehicle,
    );
...
on('Fluxcore:server:characterDeleted', (_source, characterId) => {
  ...
  for (const vehicle of owned) {
    const response = globalThis.exports.fluxcore_inventory.DeleteContainer(
      `vehicle:${vehicle.id}`,
    );
    if (!response?.ok) {
      runtime.log(
...
onNet('fluxcore_vehicles:server:request', () => {
...
onNet('fluxcore_vehicles:server:spawn', (vehicleId, garageId) => {
  const source = Number(global.source);
...
onNet('fluxcore_vehicles:server:store', (networkId, garageId, properties) => {
  const source = Number(global.source);
...
onNet('fluxcore_vehicles:server:trunk', (networkId) => {
  const source = Number(global.source);
  ...
  handle(source, () => {
    ...
    const response = globalThis.exports.fluxcore_inventory.OpenInventory(
      source,
...
onNet('fluxcore_vehicles:server:toggleLock', (networkId) => {
  const source = Number(global.source);
  ...
  handle(source, () => {
    const ped = playerPed(source);
    const entity = entityFromNetwork(networkId);
    const managed = database.getByNetwork(Number(networkId));
    let locked;
    if (managed?.state === 'out') {
      locked = vehicles.toggleLock(
        source,
        networkId,
        coordinates(ped),
        coordinates(entity),
      ...
      const playerPosition = coordinates(ped);
      const vehiclePosition = coordinates(entity);
      ...
      const access = genericVehicleAccess.get(id);
      const isDriver = Number(GetPedInVehicleSeat(entity, -1)) === ped;
      if (
        distance > config.entityDistance ||
        (!isDriver &&
          (!access || access.source !== source || access.entity !== entity))
      ) {
        throw new VehiclesError(
          'VEHICLE_ACCESS_DENIED',
          'you need a key or must first use the vehicle as its driver',
        );
      }
      if (isDriver) {
        genericVehicleAccess.set(id, { source, entity });
      }
      locked = !(genericLockStates.get(id) ?? false);
      genericLockStates.set(id, locked);
    }
    SetVehicleDoorsLocked(entity, locked ? 2 : 1);
    runtime.emitAll(
      'fluxcore_vehicles:client:lockChanged',
      Number(networkId),
      locked,
    );
    notify(
...
onNet('fluxcore_vehicles:server:toggleEngine', (networkId, requestedState) => {
  const source = Number(global.source);
  ...
  handle(source, () => {
    ...
    const id = Number(networkId);
    const enabled = requestedState === true;
...
onNet('fluxcore_vehicles:server:initialized', (networkId) => {
  const source = Number(global.source);
  ...
  if (
    ...
    Entity(entity).state.set('Fluxcore:initVehicle', null, true);
  }
});
