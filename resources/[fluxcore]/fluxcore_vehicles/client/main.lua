...
RegisterNetEvent('fluxcore_vehicles:client:update', function(snapshot)
    vehicles = snapshot and snapshot.vehicles or {}
    TriggerEvent('fluxcore_vehicles:client:updated', copy(snapshot))
end)

RegisterNetEvent('fluxcore_vehicles:client:message', function(text, kind, code)
...
RegisterNetEvent('fluxcore_vehicles:client:spawned', function(id, vehicle)
...
AddStateBagChangeHandler('Fluxcore:initVehicle', nil, function(bagName, key, value)
    ...
    CreateThread(function()
        ...
        while GetGameTimer() < deadline do
            ...
            if entity ~= 0 and DoesEntityExist(entity)
                ...
                TriggerServerEvent(
                    'fluxcore_vehicles:server:initialized',
...
RegisterNetEvent('fluxcore_vehicles:client:lockChanged', function(id, locked)
...
RegisterCommand('garage', function(_, args)
    ...
    if action == 'spawn' then
        ...
        TriggerServerEvent('fluxcore_vehicles:server:spawn', args[2], garageId)
        return
    end
    if action == 'store' then
        ...
        TriggerServerEvent(
            'fluxcore_vehicles:server:store',
            id,
            garageId,
