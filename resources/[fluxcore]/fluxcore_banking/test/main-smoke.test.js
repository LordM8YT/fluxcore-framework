...
test('Cfx wiring boots and registers banking APIs', (t) => {
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-bank-main-'));
  ...
  Object.assign(globalThis.exports, { fluxcore_core: coreExports });
  ...
  t.after(() => {
    handlers.get('onResourceStop')?.('fluxcore_banking');
  ...
  assert.equal(netHandlers.has('fluxcore_banking:server:request'), true);
  assert.equal(handlers.has('Fluxcore:server:playerLoaded'), true);
