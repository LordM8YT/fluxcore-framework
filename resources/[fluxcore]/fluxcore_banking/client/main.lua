...
local function message(text, kind)
    ...
    TriggerEvent('chat:addMessage', {
        color = color,
        args = { 'Fluxcore Bank', tostring(text) }
    })
    print(('[fluxcore_banking] %s'):format(tostring(text)))
end
...
RegisterNetEvent('fluxcore_banking:client:response', function(requestId, response)
...
RegisterNetEvent('fluxcore_banking:client:update', function(value)
    snapshot = copy(value)
    TriggerEvent('fluxcore_banking:client:updated', copy(snapshot))
end)

RegisterNetEvent('Fluxcore:client:playerLoggedOut', function()
    snapshot = nil
    TriggerEvent('fluxcore_banking:client:updated', nil)
end)
...
RegisterCommand('bank', function()
    ...
    TriggerEvent('fluxcore_banking:client:open', copy(snapshot))
end, false)
    ...
    for requestId, resolver in pairs(pending) do
        resolver({
            ...
            error = {
                ...
                message = 'Fluxcore Banking stopped.'
            }
        })
        pending[requestId] = nil
    end
