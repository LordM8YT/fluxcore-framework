...
Persistent freemode character appearance for Fluxcore Framework. The resource is
...
model follows the character gender from `fluxcore_core`, and appearance is
reapplied after every `playerSpawned` event.

## Integration

The local `fluxcore_appearance:client:openRequested` event receives the current
appearance when `/appearance` is used. An editor can submit its result with:

```lua
exports.fluxcore_appearance:SaveAppearance(appearance)
```
