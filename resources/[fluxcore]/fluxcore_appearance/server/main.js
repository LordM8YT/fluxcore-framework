...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_appearance] [${level}] ${message}`;
    if (level === 'error') {
...
function notifyError(source, response) {
  if (!response.ok) {
    runtime.emitClient(
      Number(source),
      'fluxcore_appearance:client:error',
...
onNet('fluxcore_appearance:server:request', () => {
...
onNet('fluxcore_appearance:server:save', (value) => {
  const source = Number(global.source);
...
onNet('fluxcore_appearance:server:reset', () => {
  const source = Number(global.source);
