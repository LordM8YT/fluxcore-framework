...
test('Cfx wiring boots and exposes appearance APIs', () => {
  ...
  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'Fluxcore-appearance-main-'),
  );
  ...
  try {
    ...
    assert.equal(netHandlers.has('fluxcore_appearance:server:save'), true);
    ...
    assert.equal(
      emitted.some(
        (entry) => entry.eventName === 'fluxcore_appearance:client:update',
      ),
      true,
    );
    eventHandlers.get('onResourceStop')('fluxcore_appearance');
  } finally {
