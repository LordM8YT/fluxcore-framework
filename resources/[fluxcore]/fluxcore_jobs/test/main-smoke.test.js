...
test('Cfx wiring boots and serves job events, commands, and exports', () => {
  ...
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-jobs-main-'));
  ...
  try {
    ...
    assert.equal(eventHandlers.has('Fluxcore:server:playerLoaded'), true);
    assert.equal(netHandlers.has('fluxcore_jobs:server:clock'), true);
    ...
    assert.equal(
      emitted.some((event) => event.eventName === 'fluxcore_jobs:client:update'),
      true,
    );
    ...
    context.source = 7;
    netHandlers.get('fluxcore_jobs:server:setActive')('police');
    netHandlers.get('fluxcore_jobs:server:clock')('police');
    assert.equal(
