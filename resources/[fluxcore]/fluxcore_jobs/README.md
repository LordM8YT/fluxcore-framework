...
`fluxcore_jobs` owns persistent job assignments, grades, duty state, and resource
permissions. It uses the public `fluxcore_core` API and keeps its data in a separate
SQLite database.

## Configuration

Edit `config/jobs.json` to define jobs and grades. Each grade has an explicit
permission list and payment value. Restart `fluxcore_jobs` after changing the file.

By default, permission checks require the active job to be on duty. A server
resource can override that for a single check:

```js
const allowed = exports.fluxcore_jobs.HasPermission(source, 'police.records.read', {
  requireDuty: false,
});
```
...
An identifier can be an online server source or a Fluxcore character ID. Mutating
