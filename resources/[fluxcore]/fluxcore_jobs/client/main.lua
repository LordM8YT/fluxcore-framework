...
local function message(text, kind)
    ...
    print(('[fluxcore_jobs] %s'):format(tostring(text)))
end
...
RegisterNetEvent('fluxcore_jobs:client:update', function(snapshot)
    ...
    TriggerEvent('fluxcore_jobs:client:updated', copy(snapshot))
end)

RegisterNetEvent('fluxcore_jobs:client:message', function(text, kind, code)
...
RegisterNetEvent('Fluxcore:client:playerLoaded', function()
    TriggerServerEvent('fluxcore_jobs:server:request')
end)
...
RegisterCommand('job', function(_, args)
    ...
    TriggerServerEvent('fluxcore_jobs:server:setActive', args[1])
end, false)
