...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_mdt] [${level}] ${message}`;
...
const integrations = {
  core: {
    ...
    getCharacterData(identifier) {
      return globalThis.exports.fluxcore_core.GetCharacterData(identifier);
    },
  },
  jobs: {
  ...
  vehicles: {
    getVehicles(identifier) {
      ...
      return globalThis.exports.fluxcore_vehicles.GetVehicles(identifier);
    },
  },
  dispatch: {
    getDispatch(identifier) {
      requireResource('fluxcore_dispatch');
      return globalThis.exports.fluxcore_dispatch.GetDispatch(identifier);
    },
  },
};
...
onNet('fluxcore_mdt:server:request', (requestId, method, payload) => {
