...
Police records backend for Fluxcore Framework. It includes person search, profiles,
owned vehicles, reports, warrants, BOLOs, and the current dispatch feed.

The resource contains no fixed NUI. `/mdt` emits
`fluxcore_mdt:client:open` with the versioned bootstrap contract. UI adapters use
