...
local function call(method, payload)
    ...
    TriggerServerEvent(
        'fluxcore_services:server:request',
        requestId,
        method,
        payload or {}
    )
...
RegisterNetEvent('fluxcore_services:client:response', function(requestId, response)
...
RegisterNetEvent('fluxcore_services:client:update', function(value)
    snapshot = copy(value)
    TriggerEvent('fluxcore_services:client:updated', copy(snapshot))
end)
...
RegisterCommand('invoices', function()
    ...
    if response.ok then
        ...
        TriggerEvent('fluxcore_services:client:open', copy(snapshot))
    else
