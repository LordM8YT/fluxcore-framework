...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_services] [${level}] ${message}`;
    if (level === 'error') {
...
const integrations = {
  ...
  businesses: {
    hasPermission(identifier, businessId, permission) {
      ...
      return globalThis.exports.fluxcore_businesses.HasPermission(
        identifier,
        businessId,
        permission,
      );
    ...
    creditTreasury(businessId, amount, reason, reference) {
      requireResource('fluxcore_businesses');
      return globalThis.exports.fluxcore_businesses.CreditTreasury(
        businessId,
        amount,
        reason,
        reference,
...
onNet('fluxcore_services:server:request', (requestId, method, payload) => {
  ...
  runtime.emitClient(
    source,
    'fluxcore_services:client:response',
