...
function createHarness(t, options = {}) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-phone-'));
...
test('contacts and offline-capable text messages form a private lifecycle', (t) => {
  ...
  assert.equal(
    events.some(
      (event) =>
        event.source === 8 &&
        event.eventName === 'fluxcore_phone:client:newMessage',
    ),
    true,
  );
  ...
  assert.equal(received.messages[0].body, 'Hei fra Fluxcore');
  assert.equal(
    events.some(
      (event) =>
        event.source === 7 &&
        event.eventName === 'fluxcore_phone:client:messagesRead',
    ),
    true,
  );
