...
test('Cfx wiring boots and serves the phone request channel', () => {
  ...
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'Fluxcore-phone-main-'));
  ...
  try {
    ...
    assert.equal(netHandlers.has('fluxcore_phone:server:request'), true);
    assert.equal(eventHandlers.has('Fluxcore:server:characterDeleted'), true);
    assert.equal(registeredExports.has('GetPhoneNumber'), true);

    context.source = 7;
    netHandlers.get('fluxcore_phone:server:request')(
      'smoke:1',
      'bootstrap',
      {},
    );
    const response = emitted.at(-1);
    assert.equal(response.eventName, 'fluxcore_phone:client:response');
    assert.equal(response.args[1].ok, true);
    assert.match(response.args[1].data.account.phoneNumber, /^5\d{7}$/);

    eventHandlers.get('onResourceStop')('fluxcore_phone');
  } finally {
