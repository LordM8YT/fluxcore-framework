...
`fluxcore_phone` is the text-only communication MVP for Fluxcore. It owns phone
...
The check is performed on the server through `fluxcore_inventory`. A stopped
...
`{ ok, data, error }` result envelope and supports an online source or a Fluxcore
...
- `fluxcore_phone:client:newMessage`
- `fluxcore_phone:client:messagesRead`
- `fluxcore_phone:client:contactsUpdated`
