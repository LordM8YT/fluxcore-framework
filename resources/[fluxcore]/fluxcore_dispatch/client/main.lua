...
local function call(method, payload)
    ...
    TriggerServerEvent(
        'fluxcore_dispatch:server:request',
        requestId,
        method,
        payload or {}
    )
...
local function message(text, error)
    TriggerEvent('chat:addMessage', {
        ...
        args = { 'Fluxcore Dispatch', text }
    })
end

RegisterNetEvent('fluxcore_dispatch:client:response', function(requestId, response)
    local resolver = pending[tostring(requestId)]
    if resolver then resolver(response) end
end)

RegisterNetEvent('fluxcore_dispatch:client:update', function(value)
    snapshot = copy(value)
    TriggerEvent('fluxcore_dispatch:client:updated', copy(snapshot))
end)
...
RegisterCommand('dispatch', function()
    ...
    if response.ok then
        ...
        TriggerEvent('fluxcore_dispatch:client:open', copy(snapshot))
    else
