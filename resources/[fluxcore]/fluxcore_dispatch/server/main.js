...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_dispatch] [${level}] ${message}`;
...
const integrations = {
  ...
  services: {
    getRoster() {
      requireResource('fluxcore_services');
      return globalThis.exports.fluxcore_services.GetRoster();
    },
  },
};
...
onNet('fluxcore_dispatch:server:request', (requestId, method, payload) => {
  ...
  runtime.emitClient(
    source,
    'fluxcore_dispatch:client:response',
