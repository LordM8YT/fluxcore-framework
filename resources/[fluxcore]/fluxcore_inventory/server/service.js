...
class InventoryService {
  ...
  syncContainer(containerId) {
    ...
    if (Number.isSafeInteger(source) && source > 0) {
      this.runtime.emitClient(
        source,
        'fluxcore_inventory:client:update',
  ...
  createDrop(identifier, slot, amount, position, actor = 'player') {
    ...
    try {
      ...
      this.runtime.emitAll?.('fluxcore_inventory:client:dropCreated', drop);
  ...
  removeDrop(containerId) {
    ...
    if (deleted) {
      this.runtime.emitAll?.('fluxcore_inventory:client:dropRemoved', containerId);
    }
    return deleted;
  }
