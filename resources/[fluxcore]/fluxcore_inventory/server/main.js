...
const runtime = {
  ...
  log(level, message) {
    const output = `[fluxcore_inventory] [${level}] ${message}`;
    if (level === 'error') {
...
function notifyError(source, response) {
  if (!response.ok && Number(source) > 0) {
    runtime.emitClient(
      Number(source),
      'fluxcore_inventory:client:error',
...
on('Fluxcore:server:playerLoaded', (source) => {
  ...
  runtime.emitClient(Number(source), 'fluxcore_inventory:client:drops', inventory.getDrops());
});
...
onNet('fluxcore_inventory:server:request', () => {
...
onNet('fluxcore_inventory:server:requestDrops', () => {
  const source = Number(global.source);
  if (rateLimit(source, 'drops', 1000)) {
    runtime.emitClient(source, 'fluxcore_inventory:client:drops', inventory.getDrops());
  }
});

onNet('fluxcore_inventory:server:openDrop', (dropId) => {
  const source = Number(global.source);
  ...
  if (response.ok) {
    runtime.emitClient(source, 'fluxcore_inventory:client:open', response.data);
  }
});

onNet('fluxcore_inventory:server:nui', (requestId, method, payload) => {
  const source = Number(global.source);
  ...
  if (!rateLimit(source, 'nui', 75)) {
    runtime.emitClient(
      source,
      'fluxcore_inventory:client:nuiResponse',
      id,
      {
        ok: false,
        error: {
...
onNet('fluxcore_inventory:server:move', (fromSlot, toSlot, amount) => {
  const source = Number(global.source);
...
onNet('fluxcore_inventory:server:use', (slot) => {
  const source = Number(global.source);
...
globalThis.exports('OpenInventory', (source, secondaryContainerId) => {
  ...
  if (response.ok) {
    runtime.emitClient(Number(source), 'fluxcore_inventory:client:open', response.data);
  }
  return response;
});
