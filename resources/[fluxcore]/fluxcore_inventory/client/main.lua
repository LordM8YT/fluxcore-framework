...
local function message(text, kind)
    ...
    print(('[fluxcore_inventory] %s'):format(tostring(text)))
end
...
local function closeInventory(notifyServer)
    ...
    SendNUIMessage({ action = 'Fluxcore:inventory:close' })
    TriggerEvent('fluxcore_inventory:client:closed')
    if notifyServer ~= false then
        requestSequence = requestSequence + 1
        TriggerServerEvent(
            'fluxcore_inventory:server:nui',
...
local function present(payload)
    ...
    TriggerEvent('fluxcore_inventory:client:uiOpenRequested', copy(payload))
    ...
    SendNUIMessage({
        action = 'Fluxcore:inventory:open',
        payload = payload,
        localeName = exports.fluxcore_core:GetLocale(),
        locale = {
            inventory = exports.fluxcore_core:GetLocaleData('inventory'),
            labels = exports.fluxcore_core:GetLocaleData('labels')
        }
    })
end
...
RegisterNetEvent('fluxcore_inventory:client:update', function(snapshot)
    ...
    if uiOpen then
        SendNUIMessage({
            action = 'Fluxcore:inventory:update',
            payload = { player = snapshot }
        })
    end
    TriggerEvent('fluxcore_inventory:client:updated', copy(snapshot))
end)

RegisterNetEvent('fluxcore_inventory:client:open', function(payload)
    present(payload)
end)

RegisterNetEvent('fluxcore_inventory:client:nuiResponse', function(requestId, response)
    ...
    if response and response.ok and type(response.data) == 'table'
        and response.data.contract == 'Fluxcore.inventory.bootstrap.v1' then
...
RegisterNetEvent('fluxcore_inventory:client:drops', function(entries)
    drops = {}
...
RegisterNetEvent('fluxcore_inventory:client:dropCreated', function(drop)
    if type(drop) == 'table' and drop.id and drop.position then
        drops[drop.id] = drop
    end
end)

RegisterNetEvent('fluxcore_inventory:client:dropRemoved', function(dropId)
...
RegisterNetEvent('fluxcore_inventory:client:error', function(text, code)
...
RegisterNetEvent('Fluxcore:client:playerLoaded', function()
    TriggerServerEvent('fluxcore_inventory:server:request')
    TriggerServerEvent('fluxcore_inventory:server:requestDrops')
end)
...
RegisterCommand('invslot', function(_, args)
    ...
    TriggerServerEvent(
        'fluxcore_inventory:server:move',
        tonumber(args[1]),
        tonumber(args[2]),
...
RegisterCommand('useitem', function(_, args)
    ...
    TriggerServerEvent('fluxcore_inventory:server:use', tonumber(args[1]))
end, false)
            ...
            for dropId, drop in pairs(drops) do
                ...
                if distance < 25.0 then
                    ...
                    if distance <= 2.0 and IsControlJustReleased(0, 38)
                        ...
                        TriggerServerEvent('fluxcore_inventory:server:openDrop', dropId)
                    end
                end
            end
        end
