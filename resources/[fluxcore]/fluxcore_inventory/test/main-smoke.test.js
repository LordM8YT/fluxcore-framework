...
test('Cfx wiring boots and exposes safe inventory operations', () => {
  ...
  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'Fluxcore-inventory-main-'),
  );
  ...
  try {
    ...
    assert.equal(netHandlers.has('fluxcore_inventory:server:move'), true);
    assert.equal(netHandlers.has('fluxcore_inventory:server:nui'), true);
