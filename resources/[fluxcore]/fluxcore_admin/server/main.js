...
const runtime = {
  ...
  getPlayers() {
    return globalThis.exports.fluxcore_core
      .GetPlayers()
  ...
  log(level, message) {
    const output = `[fluxcore_admin] [${level}] ${message}`;
    if (level === 'error') {
...
const integrations = {
  core: {
    ...
    setMoney(identifier, currency, amount, reason, reference) {
      return globalThis.exports.fluxcore_core.SetMoney(
        identifier,
        currency,
        amount,
        reason,
  ...
  jobs: {
    assignJob(identifier, jobName, grade) {
      ...
      return globalThis.exports.fluxcore_jobs.AssignJob(
        identifier,
        jobName,
        grade,
      );
...
onNet('fluxcore_admin:server:request', (requestId, method, payload) => {
  ...
  runtime.emitClient(
    source,
    'fluxcore_admin:client:response',
